...
def run_mnasnet():
    ...
    train_loader,valid_loader, test_loader, class_size = load_mnist('data/', batch_size=240)
    ...
    train_loader,valid_loader, test_loader, class_size = load_cifar100('data/', batch_size=64)
    ...
    train_loader,valid_loader, test_loader, class_size = load_cars('data/', batch_size=64)
    dnn = DNN(device, model_name, class_size, batch_size=64, max_epochs=100, log_file_name=model_name, models_save_dir=model_dump_dir, optim_param=optim_param, train_full=True)
    dnn.train(train_loader, valid_loader)
    dict_params = dnn.get_dict()
    ...
    with open(final_logs, 'a+') as fp:
        json_string = json.dumps(dict_params)
        fp.write(json_string + "\n")
    train_loader,valid_loader, test_loader, class_size = load_FashionMNIST('data/', batch_size=120)
    ...
    train_loader,valid_loader, test_loader, class_size = load_KMNIST('data/', batch_size=120)
    ...
    train_loader,valid_loader, test_loader, class_size = load_cifar10('data/', batch_size=120)
