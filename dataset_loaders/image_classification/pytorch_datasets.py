...
def load_mnist(data_dir, transform=None, validation_split=0.2, batch_size=256, add_feature=False):
    ...
    if add_feature == True:
        ...
        word_embed = get_word_list_embeddings('mnist', labels)
    ...
    return {'train_loader': train_loader, 'validation_loader': valid_loader,
            'test_loader': test_loader, 'num_classes': 10, 'name': 'mnist'}
...
def load_KMNIST(data_dir, transform=None, validation_split = 0.2, batch_size=256, add_feature=False):
    ...
    if add_feature == True:
        ...
        word_embed = get_word_list_embeddings('kmnist', labels)
    ...
    print("Got total number test data ", len(test_dataset))
    return {'train_loader': train_loader, 'validation_loader': valid_loader,
            'test_loader': test_loader, 'num_classes': 10, 'name': 'kmnist'}
...
def load_cifar10(data_dir, transform=None, validation_split = 0.2, batch_size=256, add_feature=False):
    ...
    download = True
    if os.path.exists(os.path.join(data_dir,'cifar-10-python.tar.gz')):
        download = False
    # load the dataset
    ...
    if add_feature == True:
        ...
        train_size = len(train_dataset)
        num_classes = 10
        fp = open(data_dir + '/cifar-10-batches-py/batches.meta','rb')
        labels = list(pickle.load(fp)['label_names'])
        word_embed = get_word_list_embeddings('cifar10', labels)
        add_vector_to_file("Cifar10",img_h, img_w, train_size, num_classes, word_embed)
    ...
    print("Got total number test data ", len(test_dataset))
    return {'train_loader': train_loader, 'validation_loader': valid_loader,
            'test_loader': test_loader, 'num_classes': 10, 'name': 'cifar10'}
